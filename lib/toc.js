...
(function () {
  ...
  function buildSidebar(items) {
    ...
    // Settings gear: opens the display-settings popover (font, size, width,
    // theme). Built by lib/settings.js, which persists each choice and applies
    // it on load.
    if (window.MarkdownStudio && window.MarkdownStudio.buildSettingsButton) {
      try {
        actions.appendChild(window.MarkdownStudio.buildSettingsButton());
      } catch (e) {
        console.warn('[markdown-studio] settings button failed to mount:', e);
      }
    }
    ...
    editBtn.addEventListener('click', () => {
      const mv = window.MarkdownStudio;
      if (!mv || !mv.mountEditor) {
        console.warn('[markdown-studio] editor module not loaded');
        return;
      }
    ...
    printBtn.className = 'md-export-item';
    ...
    printBtn.addEventListener('click', async () => {
      ...
      try {
        const rerender = window.MarkdownStudio && window.MarkdownStudio.rerenderMermaidForTheme;
        if (rerender) {
          try { await rerender('light'); }
          catch (e) { console.warn('[markdown-studio] mermaid pre-print rerender failed:', e); }
        }
        // Append a print-only "Comments" appendix so annotations show up in
        // the PDF (the screen UI / highlights are hidden in print). Removed in
        // cleanup so the live page is untouched.
        let appendix = null;
        const article = document.body.querySelector(':scope > article.markdown-studio-content');
        if (article && window.MarkdownStudio && window.MarkdownStudio.buildPrintAppendix) {
          // Defensive: drop any appendix a previous print left behind (e.g. if
          // afterprint never fired) so we never stack two of them in the PDF.
          article.querySelectorAll(':scope > .md-comment-print-appendix').forEach((n) => n.remove());
          try {
            appendix = window.MarkdownStudio.buildPrintAppendix();
            if (appendix) article.appendChild(appendix);
          } catch (e) { console.warn('[markdown-studio] comment appendix failed:', e); appendix = null; }
        }
        // Idempotent cleanup. afterprint is not guaranteed to fire on every
        // cancel path, so the finally below runs cleanup too; the flag keeps it
        // to exactly one execution (and stops a second Print from stacking).
        let cleaned = false;
        const cleanup = async () => {
          if (cleaned) return;
          cleaned = true;
          window.removeEventListener('afterprint', cleanup);
          if (appendix && appendix.parentNode) { appendix.parentNode.removeChild(appendix); appendix = null; }
          if (rerender) {
            try { await rerender('screen'); }
            catch (e) { console.warn('[markdown-studio] mermaid post-print rerender failed:', e); }
          }
        };
        window.addEventListener('afterprint', cleanup);
        // window.print() blocks until the dialog is dismissed in Chrome, so the
        // finally guarantees cleanup even when afterprint is skipped.
        try { window.print(); }
        catch (e) { console.warn('[markdown-studio] print failed:', e); }
        finally { await cleanup(); }
      } finally {
        printBtn.disabled = false;
      }
    });
    // Word export: walks the rendered article, builds a docx in-browser via
    // JSZip + hand-built OOXML, embeds math as editable OMML (KaTeX MathML
    // -> mathml2omml -> post-processors -> <m:oMathPara>). See
    // lib/exporter-docx.js for the conversion pipeline.
    const wordBtn = document.createElement('button');
    wordBtn.className = 'md-export-item';
    wordBtn.type = 'button';
    wordBtn.setAttribute('role', 'menuitem');
    wordBtn.title = 'Export as Word (.docx) with editable equations';
    wordBtn.setAttribute('aria-label', 'Export this page as Word document');
    wordBtn.textContent = 'Word (.docx)';
    wordBtn.addEventListener('click', async () => {
      closeExportMenu(false);
      const mv = window.MarkdownStudio;
      if (!mv || !mv.exportToWord) {
        console.warn('[markdown-studio] exporter-docx module not loaded');
        return;
      }
      // Always look up the rendered article from the DOM -- buildSidebar's
      // scope doesn't see mountTOC's `article` parameter. Scope to a direct
      // body child so we never grab the editor's live-preview article.
      const articleEl = document.body.querySelector(':scope > article.markdown-studio-content');
      if (!articleEl) {
        console.warn('[markdown-studio] no article in body; nothing to export');
        return;
      }
      wordBtn.disabled = true;
      // Mermaid renders with the dark theme when the page is dark, but the
      // exporter rasterises each diagram onto an opaque white background --
      // dark-theme labels would be near-invisible in the .docx. Re-render the
      // diagrams light for the export, then restore the on-screen theme.
      // Mirrors the PDF button's pre/post-print rerender.
      const rerender = mv.rerenderMermaidForTheme;
      if (rerender) {
        try { await rerender('light'); }
        catch (e) { console.warn('[markdown-studio] mermaid pre-export rerender failed:', e); }
      }
      try {
        await mv.exportToWord(articleEl);
      } catch (e) {
        console.warn('[markdown-studio] Word export failed:', e);
      } finally {
        if (rerender) {
          try { await rerender('screen'); }
          catch (e) { console.warn('[markdown-studio] mermaid post-export rerender failed:', e); }
        }
        wordBtn.disabled = false;
      }
    });

    // Export dropdown: one toolbar button that opens a small menu with the
    // PDF and Word actions, instead of two separate toolbar buttons.
    const exportBtn = document.createElement('button');
    exportBtn.className = 'md-toc-export';
    exportBtn.type = 'button';
    exportBtn.title = 'Export this document (PDF or Word)';
    ...
    exportMenu.className = 'md-export-menu';
    exportMenu.setAttribute('role', 'menu');
    exportMenu.setAttribute('aria-label', 'Export format');
    exportMenu.appendChild(printBtn);
    exportMenu.appendChild(wordBtn);
    ...
    function onExportKey(e) {
      if (e.key === 'Escape') { e.preventDefault(); closeExportMenu(true); }
    }
    function openExportMenu() {
      ...
      document.addEventListener('mousedown', onExportOutside, true);
    ...
    function closeExportMenu(restoreFocus) {
      if (!exportOpen) return;
      exportOpen = false;
      document.removeEventListener('mousedown', onExportOutside, true);
    ...
    exportBtn.addEventListener('click', () => {
      if (exportOpen) closeExportMenu(true); else openExportMenu();
    });
  ...
  const STORAGE_KEY = 'markdown-studio-toc-hidden';
  ...
  function mountTOC(article) {
    ...
    document.body.classList.add('markdown-studio-with-toc');
  ...
  window.MarkdownStudio.mountTOC = mountTOC;
})();
