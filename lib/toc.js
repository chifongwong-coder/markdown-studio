...
(function () {
  ...
  function attachInteractions(nav) {
    nav.addEventListener('click', (e) => {
      ...
      } else if (t.classList.contains('md-toc-hide')) {
        setSidebarHidden(true);
      }
    });
  }

  /** 收起/展开整个 TOC 侧栏。状态用 localStorage 记忆。 */
  ...
  /** 创建左侧"展开"把手按钮，常驻 DOM，CSS 控制何时显示。 */
  function buildHandle() {
    const handle = document.createElement('button');
    handle.className = 'md-toc-handle';
    handle.type = 'button';
    handle.title = '展开目录';
    handle.setAttribute('aria-label', '展开目录');
    handle.textContent = '»';
    handle.addEventListener('click', () => setSidebarHidden(false));
    return handle;
  }
  ...
  function mountTOC(article) {
    ...
    // 应用上次保存的「收起/展开」状态
    if (loadSidebarHidden()) document.body.classList.add('md-toc-hidden');
