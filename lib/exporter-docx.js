...
(function () {
  ...
  function mathmlToOmml(mathElement) {
    ...
    let omml;
    // mml2omml emits `Type not supported: <tag>` via console.log for tags it
    // skips (annotation, mpadded, ...). These are intentional (the tags are
    // metadata or styling-only) but show up as a noisy ~60-entry "error" wall
    // in DevTools for a typical math-heavy article. Silence console.log just
