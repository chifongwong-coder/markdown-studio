...
(function () {
  ...
  function normalizeMathVariants(mml) {
    // Pass 1: variants we map to a pre-styled Unicode glyph (script,
    // double-struck, fraktur, sans-serif on ASCII letters).
    mml = mml.replace(
    ...
    // Pass 2: mathvariant="normal" -- KaTeX puts this on \mathrm{} content
    // AND on symbol glyphs (∞ ∇ π …). mml2omml doesn't recognise "normal"
    // and emits the buggy <m:sty m:val="undefined"/> placeholder, so Word
    // falls back to body-text font for that run. Convert to <mtext>, which
    // mml2omml renders as a non-italic OMML run -- matching the LaTeX
    // \mathrm intent for letters and harmless for symbols (already upright).
    mml = mml.replace(
      /<mi(?:\s+[^>]*?)?\s*mathvariant="normal"(?:\s+[^>]*?)?\s*>([^<]+)<\/mi>/g,
      '<mtext>$1</mtext>'
    );
    // Pass 3: anything else (italic, bold-italic, monospace, ...) we don't
    // have a clean mapping for. Strip the attribute so mml2omml renders the
    // content with its default styling rather than emitting `m:val="undefined"`.
    mml = mml.replace(
      /(<mi[^>]*?)\s+mathvariant="[^"]*"([^>]*?>)/g,
      '$1$2'
    );
  ...
  function walkInline(el, ctx, fmt) {
    ...
    for (let i = 0; i < el.childNodes.length; i++) {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (t) appendAll(out, runsForTextNode(node, t, fmt, ctx));
  ...
  // ---- native Word review comments ----------------------------------------
  //
  // We place each comment as a real Word comment: <w:commentRangeStart/> and
  // <w:commentRangeEnd/> wrap the quoted text in the body, a <w:commentReference>
  // run follows, and the comment body lives in word/comments.xml. The anchor
  // is matched by *text-node identity* -- the comment's resolved Range points
  // at the same DOM text nodes walkInline iterates. That only works on the
  // paths that walk the ORIGINAL nodes (paragraphs, headings, blockquotes,
  // figcaptions, fallback). buildList clones nodes (walkInlineNodes), and
  // code/tables/math/mermaid take non-text paths -- comments anchored there
  // can't be placed inline, so they fall back to a trailing "Comments" block
  // (see buildDocumentXml) which still pairs every range so the file is valid.

  /** A text node is inline-anchorable only when none of its ancestors up to
   *  the article is a clone-path or non-text-path container. */
  function isCommentAnchorable(node, article) {
    if (!node || node.nodeType !== Node.TEXT_NODE || !article.contains(node)) return false;
    let el = node.parentElement;
    while (el && el !== article) {
      const tag = el.tagName.toLowerCase();
      if (tag === 'pre' || tag === 'code' || tag === 'ul' || tag === 'ol' ||
          tag === 'li' || tag === 'table' || tag === 'figure') return false;
      if (el.classList && (
        el.classList.contains('katex') ||
        el.classList.contains('md-mermaid') ||
        el.classList.contains('md-toc') ||
        el.classList.contains('md-toc-handle') ||
        el.classList.contains('md-toc-hide') ||
        el.classList.contains('md-file-tip'))) return false;
      el = el.parentElement;
    }
    return true;
  }

  function pushBoundary(map, node, entry) {
    let arr = map.get(node);
    if (!arr) { arr = []; map.set(node, arr); }
    arr.push(entry);
  }
  ...
  /** Assign each non-orphan comment a numeric id, decide whether it can be
   *  anchored inline, and index its start/end boundaries by text node. Results
   *  are stashed on ctx for walkInline + buildDocumentXml + buildCommentsXml. */
  function prepareComments(raw, article, ctx) {
    ctx.commentList = [];
    ctx.commentStarts = new Map();
    ctx.commentEnds = new Map();
    ctx.commentDate = nowIso();
    if (!raw || !raw.length) return;
    let wid = 0;
    for (const c of raw) {
      if (c.orphan || !c.range) continue;
      let sc, so, ec, eo;
      try {
        sc = c.range.startContainer; so = c.range.startOffset;
        ec = c.range.endContainer; eo = c.range.endOffset;
      } catch (e) { continue; }
      if (!sc || !ec) continue;
      const anchorable = isCommentAnchorable(sc, article) && isCommentAnchorable(ec, article);
      if (anchorable) {
        pushBoundary(ctx.commentStarts, sc, { wordId: wid, offset: so });
        pushBoundary(ctx.commentEnds, ec, { wordId: wid, offset: eo });
      }
      ctx.commentList.push({
        wordId: wid, body: c.body || '', quote: c.quote || '',
        heading: c.heading || null, anchorable: anchorable,
      });
      wid++;
    }
  }
  ...
  function clampOffset(o, len) {
    o = o | 0;
    return o < 0 ? 0 : (o > len ? len : o);
  }

  /** Emit the runs for a single text node, splicing in commentRangeStart /
   *  commentRangeEnd / commentReference at any comment boundary that lands in
   *  this node. With no boundaries it's just one makeRun -- the common case. */
  function runsForTextNode(node, text, fmt, ctx) {
    const starts = ctx && ctx.commentStarts && ctx.commentStarts.get(node);
    const ends = ctx && ctx.commentEnds && ctx.commentEnds.get(node);
    if (!starts && !ends) return [makeRun(text, fmt)];
    const points = [];
    if (starts) for (const s of starts) points.push({ offset: clampOffset(s.offset, text.length), kind: 'start', wordId: s.wordId });
    if (ends) for (const e of ends) points.push({ offset: clampOffset(e.offset, text.length), kind: 'end', wordId: e.wordId });
    // At a shared offset, close ranges before opening new ones.
    points.sort((a, b) => a.offset - b.offset || (a.kind === b.kind ? 0 : a.kind === 'end' ? -1 : 1));
    const parts = [];
    let cursor = 0;
    for (const p of points) {
      if (p.offset > cursor) { parts.push(makeRun(text.slice(cursor, p.offset), fmt)); cursor = p.offset; }
      if (p.kind === 'start') {
        parts.push('<w:commentRangeStart w:id="' + p.wordId + '"/>');
      } else {
        parts.push('<w:commentRangeEnd w:id="' + p.wordId + '"/>');
        parts.push('<w:r><w:commentReference w:id="' + p.wordId + '"/></w:r>');
      }
    }
    if (cursor < text.length) parts.push(makeRun(text.slice(cursor), fmt));
    return parts;
  }
  ...
  function blockToParagraphs(el, ctx) {
    ...
    // render.js promotes a stand-alone display formula to a top-level
    // <div class="math-display">, so the <p> branch above never sees it.
    // Emit it as its own paragraph wrapped in <m:oMathPara> -- otherwise the
    // generic fallback below descends into the inner .katex and produces an
    // INLINE <m:oMath>, dropping the display (centered, own-line) formatting.
    if (el.classList && el.classList.contains('math-display')) {
      const mathEl = el.querySelector('.katex-mathml math');
      if (mathEl) {
        const omml = mathmlToOmml(mathEl);
        if (omml) return ['<w:p><m:oMathPara>' + omml + '</m:oMathPara></w:p>'];
      }
      // MathML missing (e.g. KaTeX render error span): don't drop the block,
      // fall back to walking it inline so its text still lands in the doc.
      const runs = walkInline(el, ctx);
    ...
    // --- blockquote -> one paragraph per child, with a left border and
    //                   indent (matches Word's standard 'Quote' style).
    //   pPr child order per CT_PPrBase: pBdr (9) before ind (23).
    if (tag === 'blockquote') {
      const quotePPr =
        '<w:pPr>' +
          '<w:pBdr>' +
            '<w:left w:val="single" w:sz="24" w:space="8" w:color="BFBFBF"/>' +
          '</w:pBdr>' +
          '<w:ind w:left="720"/>' +
        '</w:pPr>';
      const out = [];
      for (let i = 0; i < el.children.length; i++) {
        ...
        // Same pBdr on each child paragraph -- Word renders consecutive
        // matching-bordered paragraphs as one visually continuous block.
        out.push('<w:p>' + quotePPr + runs.join('') + '</w:p>');
      }
      return out;
    }
    ...
    // --- tables ---
    // content.js wraps every <table> in <div class="md-table-wrap"> so
    // wide tables can scroll horizontally in narrow viewports. The
    // walker sees the wrapper, not the table; unwrap and dispatch.
    // Assumes the wrapper contains only the <table>; if a future
    // version of content.js puts a caption / sibling inside, those
    // would be dropped here -- revisit if that happens.
    if (tag === 'div' && el.classList && el.classList.contains('md-table-wrap')) {
      const inner = el.querySelector('table');
      if (inner) return blockToParagraphs(inner, ctx);
    }
    if (tag === 'table') {
      // Empty paragraph after the table is required by the OOXML body
      // schema (a <w:tbl> can't be the last block before <w:sectPr>).
      // Use the long-form <w:p></w:p>: Word for Mac has been observed
      // to drop the cursor-park behaviour for self-closed <w:p/>.
      return [buildTable(el, ctx), '<w:p></w:p>'];
    }
  ...
  // Extract image bytes by painting the loaded <img> onto a canvas and
  // reading the PNG blob back. This works on file:// pages where fetch()
  // is blocked by CORS (each file:// URL is a unique security origin in
  // Chrome) but the <img> element already holds the bytes from its
  // normal browser load. PNG re-encode loses JPEG-source quality
  // slightly; for the docx-embed use case that's acceptable.
  async function imageBytesFromCanvas(img) {
    try {
      ...
      if (!blob) {
        // toBlob returns null when the canvas is tainted (cross-origin
        // image without CORS headers). Shouldn't happen with our
        // file-URL access perms but worth logging when it does.
        console.warn('[markdown-studio] canvas.toBlob returned null for', img.src);
        return null;
      }
  ...
  async function imageBytesFromFetch(img) {
    try {
      ...
      if (!mime || !MIME_TO_EXT[mime]) return null;
      return {
        bytes: bytes,
        mime: mime,
  ...
  // Walk the article, extract bytes for every <img> we can handle, and
  // return a Map<src, {bytes, mime, ext, w, h}>. Strategy:
  //   - Skip broken images silently (img.complete && naturalWidth === 0).
  //   - file:// images go through canvas (fetch is CORS-blocked here).
  //   - http(s) images try fetch first to preserve the original format;
  //     fall back to canvas if fetch failed for any reason.
  //   - SVGs are skipped (canvas might lose vector fidelity, Word's SVG
  //     support is inconsistent across versions); walker falls back to
  //     italic alt-text.
  //   - Same src appearing multiple times: extracted once, referenced
  //     separately in the docx.
  async function prefetchImages(article) {
    ...
    for (const img of imgs) {
      ...
      // Broken / not-loaded images: skip silently. The browser already shows
      // the broken-image icon; embedding an empty PNG would be confusing.
      if (!img.complete || img.naturalWidth === 0) continue;
      // SVG sources can't safely be PNG-rasterised here (we'd lose vector
      // sharpness and break gradients); leave them to the alt-text fallback.
      if (/\.svg(\?|#|$)/i.test(src)) continue;

      const isFile = src.indexOf('file:') === 0;
      let data = null;
      if (isFile) {
        data = await imageBytesFromCanvas(img);
      } else {
        data = await imageBytesFromFetch(img);
        if (!data) data = await imageBytesFromCanvas(img);
      }
  ...
  function buildTable(tableEl, ctx) {
    ...
    for (const tr of rows) {
      ...
      for (const cell of tr.children) {
        ...
        // Every <w:p> in a <w:tc> needs at least one <w:r> for Word's
        // table renderer to assign the cell a non-zero height. Cells whose
        // only content was inline math (`<m:oMath>` siblings of the
        // paragraph) would otherwise collapse, taking the whole row with
        // them. Prepend an empty <w:r> when no run is present, before
        // appending the math/other content.
        let inner;
        if (runs.length === 0) {
          inner = '<w:r><w:t xml:space="preserve"> </w:t></w:r>';
        } else {
          const hasRun = runs.some(function (r) { return r.indexOf('<w:r') === 0; });
          inner = hasRun ? runs.join('') : ('<w:r><w:t xml:space="preserve"></w:t></w:r>' + runs.join(''));
        }
    ...
    const tblPr =
      '<w:tblPr>' +
        // Explicit total width matching the gridCol sum.
        '<w:tblW w:w="' + (colW * colCount) + '" w:type="dxa"/>' +
        '<w:tblBorders>' +
        ...
        '</w:tblBorders>' +
        // Use the declared gridCol widths instead of auto-shrink.
        '<w:tblLayout w:type="fixed"/>' +
        // tblLook is the conditional-formatting hint Word relies on to
        // decide which corner / row / column gets the "header" styling.
        // Real Word docs always include it; without it, some Word versions
        // refuse to lay out the table at all.
  ...
  function buildDocumentXml(article, ctx) {
    ...
    // Enforce the comment pairing invariant. A comment is valid only if the
    // body carries EXACTLY one commentRangeStart + one commentRangeEnd + one
    // commentReference for its id. Anchorability was decided per-endpoint, but
    // actual marker emission depends on which DOM export path each endpoint
    // took -- a range spanning a non-text path (display math, image, table
    // cell, a cloned list item) can leave an unbalanced marker. An orphaned
    // range or reference makes Word declare the .docx corrupt and drop every
    // comment, so we strip any stray markers for such ids and re-emit them
    // intact in a trailing "Comments" block. Result: every comment is either
    // fully inline or fully in the trailing block -- never half-placed.
    if (ctx.commentList && ctx.commentList.length) {
      // Tally every id's markers in a SINGLE pass over the body (O(bodyLength),
      // not O(comments x bodyLength)). The reference is always emitted wrapped
      // as <w:r><w:commentReference/></w:r> at both emit sites, and we count
      // THAT wrapped form -- the exact string the strip below removes -- so the
      // balance test and the strip can never disagree.
      const startCount = new Map(), endCount = new Map(), refCount = new Map();
      const reMark = /<w:comment(RangeStart|RangeEnd) w:id="(\d+)"\/>/g;
      const reRef = /<w:r><w:commentReference w:id="(\d+)"\/><\/w:r>/g;
      let mm;
      while ((mm = reMark.exec(body)) !== null) {
        const map = mm[1] === 'RangeStart' ? startCount : endCount;
        map.set(mm[2], (map.get(mm[2]) || 0) + 1);
      }
      while ((mm = reRef.exec(body)) !== null) {
        refCount.set(mm[1], (refCount.get(mm[1]) || 0) + 1);
      }
      const fallback = [];
      for (const c of ctx.commentList) {
        const id = String(c.wordId);
        const balanced = c.anchorable &&
          startCount.get(id) === 1 && endCount.get(id) === 1 && refCount.get(id) === 1;
        if (!balanced) {
          const sStart = '<w:commentRangeStart w:id="' + c.wordId + '"/>';
          const sEnd = '<w:commentRangeEnd w:id="' + c.wordId + '"/>';
          const sRefRun = '<w:r><w:commentReference w:id="' + c.wordId + '"/></w:r>';
          if (body.indexOf(sStart) >= 0) body = body.split(sStart).join('');
          if (body.indexOf(sEnd) >= 0) body = body.split(sEnd).join('');
          if (body.indexOf(sRefRun) >= 0) body = body.split(sRefRun).join('');
          fallback.push(c);
        }
      }
      if (fallback.length) {
        const fb = [];
        fb.push(makeParagraph(
          [makeRun('Comments', { bold: true, size: 28 })],
          { spacingBefore: 360, spacingAfter: 120 }));
        for (const c of fallback) {
          const label = c.quote || (c.heading && c.heading.text) || 'comment';
          fb.push(makeParagraph([
            '<w:commentRangeStart w:id="' + c.wordId + '"/>',
            makeRun(label, { italic: true }),
            '<w:commentRangeEnd w:id="' + c.wordId + '"/>',
            '<w:r><w:commentReference w:id="' + c.wordId + '"/></w:r>',
          ]));
        }
        body += fb.join('');
      }
  ...
  function buildCommentsXml(ctx) {
    const items = (ctx.commentList || []).map((c) => {
      const body = String(c.body || '');
      const paras = body.split('\n').map((line) =>
        '<w:p><w:r><w:t xml:space="preserve">' + escXml(line) + '</w:t></w:r></w:p>'
      ).join('') || '<w:p/>';
      return '<w:comment w:id="' + c.wordId + '"'
        + ' w:author="Markdown Studio"'
        + (ctx.commentDate ? ' w:date="' + escXml(ctx.commentDate) + '"' : '')
        + ' w:initials="MS">' + paras + '</w:comment>';
    }).join('');
    return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
  ...
  function buildContentTypesXml(hasComments) {
    ...
    return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
      ...
      +   (hasComments ? '<Override PartName="/word/comments.xml" ContentType="' + COMMENTS_CT + '"/>' : '')
      + '</Types>';
  }
  ...
  function buildDocumentRelsXml(ctx, hasComments) {
    ...
    // comments.xml is a part relationship only -- the body references comments
    // by w:id (commentReference), not r:id, so a non-numeric id won't clash
    // with image / hyperlink rIds allocated in ctx.relationships.
    if (hasComments) {
      body += '<Relationship Id="rIdComments" Type="' + COMMENTS_REL + '" Target="comments.xml"/>';
    }
  ...
  async function buildDocxBlob(article) {
    ...
    // Pull the current comments and index their anchors BEFORE walking the
    // document, so walkInline can splice review-comment markers into the runs.
    const rawComments = (window.MarkdownStudio && window.MarkdownStudio.getComments)
      ? window.MarkdownStudio.getComments() : [];
    prepareComments(rawComments, article, ctx);
    const hasComments = ctx.commentList.length > 0;
    ...
    wordDir.folder('_rels').file('document.xml.rels', buildDocumentRelsXml(ctx, hasComments));
    wordDir.file('numbering.xml', buildNumberingXml(ctx));
    wordDir.file('footnotes.xml', buildFootnotesXml(ctx));
    if (hasComments) wordDir.file('comments.xml', buildCommentsXml(ctx));
  ...
  async function exportToWord(article, filename) {
    var name = filename || suggestFilename();
    var blob = await buildDocxBlob(article);
    var saved = await saveWithPicker(blob, name);
    if (!saved) triggerDownload(blob, name);
  }
