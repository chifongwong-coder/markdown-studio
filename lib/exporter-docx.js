...
(function () {
  ...
  function makeRun(text, fmt) {
    fmt = fmt || {};
    const parts = [];
    if (fmt.bold || fmt.italic || fmt.monospace || fmt.size || fmt.shade || fmt.color || fmt.underline) {
      parts.push('<w:rPr>');
    ...
    for (let i = 0; i < el.childNodes.length; i++) {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (tag === 'strong' || tag === 'b') {
          ...
          // Plain <sup> (subscript-style, not a citation) or footnote
          // setup failed: fall back to inline superscript text.
          appendAll(out, walkInline(node, ctx, Object.assign({}, fmt, { /* TODO: superscript */ })));
        } else if (tag === 'br') {
  ...
  function buildFootnotesXml(ctx) {
    ...
    // footnote text.
    const ids = Array.from(ctx._footnotes.keys());
    // Sort by wordId so output is stable / in citation order.
    ids.sort((a, b) => ctx._footnotes.get(a).wordId - ctx._footnotes.get(b).wordId);
  ...
  function buildNumberingXml(ctx) {
    ...
    return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
      ...
      +   '<w:abstractNum w:abstractNumId="0">' + lvls('bullet', '•') + '</w:abstractNum>'
      +   '<w:abstractNum w:abstractNumId="1">' + lvls('decimal', '') + '</w:abstractNum>'
      +   nums
      + '</w:numbering>';
  }
