...
//     fragile markdown source byte offsets; the quote + heading is plenty to
...
(function () {
  ...
  /** Snapshot of comments for the exporters. Re-anchors first so each record's
   *  live Range is current, then returns plain objects carrying the resolved
   *  Range (or null when orphaned). The Word exporter uses range node identity
   *  to place native review comments; the PDF appendix uses quote/heading/body. */
  ...
  /** Build a print-only "Comments" appendix section for the PDF export, or
   *  null when there are no comments. toc.js appends it to the article before
   *  window.print() and removes it afterward. Visible only in print (CSS). */
  function buildPrintAppendix() {
    if (!comments.length) return null;
    const sec = document.createElement('section');
    sec.className = 'md-comment-print-appendix';
    ...
    comments.forEach((c) => {
      ...
      q.className = 'md-comment-print-quote';
      const quoteTxt = c.quote.length > 120 ? c.quote.slice(0, 120) + '...' : c.quote;
      q.textContent = '"' + quoteTxt + '"' +
        (c.heading && c.heading.text ? '  (' + c.heading.text + ')' : '');

      const b = document.createElement('div');
      b.className = 'md-comment-print-body';
  ...
  window.MarkdownStudio.getComments = getComments;
  window.MarkdownStudio.buildPrintAppendix = buildPrintAppendix;
})();
