...
(function () {
  ...
  const PURIFY_CONFIG = {
    ...
    // content.js looks for it to dispatch to mermaid. ALLOW_DATA_ATTR is
    // still false (no other data-* attributes survive sanitization).
    ADD_ATTR: ['aria-hidden', 'role', 'data-mermaid-id'],
  ...
  function configureMarked() {
    ...
    if (typeof marked.use === 'function') {
      ...
      if (typeof markedFootnote !== 'undefined') {
        try { marked.use(markedFootnote()); } catch (e) { /* plugin shape mismatch */ }
      }
  ...
  /** Pull ```mermaid blocks out of the markdown source before anything else
   *  touches them. Each block becomes a placeholder div that content.js
   *  later swaps for the rendered SVG. Mermaid source is returned alongside
   *  so the rendering step can address each block by index. */
  function extractMermaid(mdText) {
    const sources = [];
    const re = /^( {0,3})```mermaid[^\n]*\n([\s\S]*?)\n {0,3}```[ \t]*$/gm;
    const text = mdText.replace(re, (_m, _indent, body) => {
      sources.push(body);
      const id = sources.length;
      // Use a regular HTML div as placeholder. It survives marked, sanitize,
      // and applyMathReplacements unchanged because none of those touch a
      // bare <div class="md-mermaid">.
      return `<div class="md-mermaid" data-mermaid-id="${id}"></div>`;
    });
    return { text, sources };
  }
